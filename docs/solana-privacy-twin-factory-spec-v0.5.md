...
- Tracks compressed commitments, roots, nullifiers, and emits REST endpoints:
  ...
  - `POST /nullifiers/:mint` → append newly spent nullifiers after proofs succeed (keeps replay protection in sync).
- Maintains Postgres for snapshots; exports Prometheus metrics.

### 5.2 Web dApp (Next.js + Phantom/Backpack)
- **Client‑side proving** using WebWorker + WebGPU where possible; Proof RPC available as a fallback.
- **Flows shipped:** Shield, Unshield (Origin / PTkn) with change outputs, cached root/nullifier discovery, note scanning via view keys, and replay-safe nullifier sync.
- **Faucet:**  
  - `NEXT_PUBLIC_FAUCET_MODE=local` + `FAUCET_MODE=local` expose a validator-backed faucet that airdrops SOL and mints origin tokens using the bootstrap mint authority.  
  - Other environments hide the faucet while the simulation wallet remains available for offline demos.
...
### 7.3 Unshield
- **Proves:** Ownership of inputs; sum(inputs) = `amount + fee + change_amount`; binds to `dest_pubkey` and `mode (Origin|PTkn)`; emits change commitments when change is non-zero; valid nullifiers.  
- **Public inputs:** `old_root, new_root, nullifiers[], change_commitment, change_amount_commitment, amount, fee, dest_pubkey, mode, mint_id, pool_id`.
...
- **Environment (.env):**
  ...
  - `DAO_MULTISIG`
  - `TIMELOCK_SECS=86400`
  - `FAUCET_MODE`, `FAUCET_KEYPAIR`, `FAUCET_RPC_URL` — enable the local validator faucet (omit on public clusters)
- **Client env:**  
  - `NEXT_PUBLIC_RPC_URL` — Solana RPC the dApp targets  
  - `NEXT_PUBLIC_PROOF_RPC_URL` — Proof service base URL  
  - `NEXT_PUBLIC_INDEXER_URL` — Photon/indexer base URL  
  - `NEXT_PUBLIC_FAUCET_MODE` — `'local'` to surface the validator faucet, `'simulation'` (default) or unset to hide it
- **Build profiles:**
...
- **Front-end E2E (Playwright):**
  - **Location:** `tests/e2e/`
  - **Purpose:** Boots the private validator, runs `bootstrap-private-devnet`, executes a real `shield → unshield` (with change + PTkn), and asserts vault/nullifier state.
  - **Command:**
    ```bash
    cd tests/e2e
    npm install
